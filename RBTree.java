...
public class RBTree<T extends Comparable, E> {
    ...
    void iteratorHelper(RedBlackNode<T,E> p, myArrayList<E> toBeReturned) {
    	if (p == null || p.getValue() == null) return;
    	for(int i = 0; i < p.getValues().size(); i++) {
    		toBeReturned.add(p.getValues().get(i));
    	}
    	iteratorHelper(p.left, toBeReturned);
    	iteratorHelper(p.right, toBeReturned);
    }
    public myArrayList<E> returnIterator(){
    	myArrayList<E> toBeReturned = new myArrayList<E>();
    	iteratorHelper(root, toBeReturned);
    	return toBeReturned;
    }
}